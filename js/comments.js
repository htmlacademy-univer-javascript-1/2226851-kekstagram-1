...
const addComments = () => {
  const newCommentsCount = MAX_COMMENTS_COUNT * maxCommentsMultiplyer;
  ...
  for(let i = 0; i < addedCommentsCount; i++) {
    if (i < newCommentsCount && i >= newCommentsCount - MAX_COMMENTS_COUNT) {
...
loadingButton.addEventListener('click', () => {
  addComments(maxCommentsMultiplyer++);
});
